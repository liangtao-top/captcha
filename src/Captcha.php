...
class Captcha
{
    ...
    public function create(string $config = null, bool $api = false): Response
    {
        ...
        // 图片宽(px)
        intval($this->imageW) || $this->imageW = $this->length * $this->fontSize * 1.5 + $this->length * $this->fontSize / 2;
        // 图片高(px)
        intval($this->imageH) || $this->imageH = $this->fontSize * 2.5;
        // 建立一幅 intval($this->imageW) x intval($this->imageH) 的图像
        $this->im = imagecreate((int)intval($this->imageW), (int)intval($this->imageH));
        // 设置背景
        ...
        for ($i = 0; $i < 10; $i++) {
            ...
            for ($j = 0; $j < 5; $j++) {
                ...
                imagestring(
                    $this->im,
                    5,
                    mt_rand(-10, intval($this->imageW)),
                    mt_rand(-10, intval($this->imageH)),
    ...
    protected function background(): void
    {
        ...
        @imagecopyresampled($this->im, $bgImage, 0, 0, 0, 0, intval($this->imageW), intval($this->imageH), $width, $height);
